...
> @brenning.2023.wiml  Interpreting machine-learning models in transformed feature space with an application to remote-sensing classification. *Machine Learning*, 112, 3455--3471, <https://doi.org/10.1007/s10994-023-06327-8>
...
```{r parallelization, warning = FALSE}
library("future")
...
knitr::include_graphics("figures/rockglacier.jpg")
```
...
```{r trainrf, cache = TRUE, cache.extra = do_cache()}
fit <- randomForest::randomForest(formula = fo, data = d, importance = TRUE)
```

```{r iml, cache = TRUE, cache.extra = do_cache(), warning = FALSE}
...
```{r strucpcawarp, cache = TRUE, cache.extra = do_cache()}
...
```{r strucpcawarpfit, cache=TRUE, cache.extra=do_cache()}
...
```{r strucpcapred, echo = FALSE, results = "hide", cache = TRUE, cache.extra = do_cache()}
...
```{r strucpcaimp, cache = TRUE, cach.extra = do_cache(), warning = FALSE}
...
```{r strucpcaimp10, cache = TRUE, cache.extra = do_cache(), warning = FALSE}
...
```{r strucpcaale, cache = TRUE, cache.extra = do_cache(), warning = FALSE}
...
```{r strucpcapdp2d, cache = TRUE, cache.extra = do_cache(), warning = FALSE}
